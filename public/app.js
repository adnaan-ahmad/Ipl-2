...
function onLoad() {
  fetch(`/economyBowler?year=2008`)
    .then(r => r.json())
    .then(data => visualizeEconomyBowler(data, 2008));
}

//var year1 = 2008
function fetchEconomyRate(){
  var year = document.getElementById('selectyear')
  //console.log(year)
  var year1 = year.options[year.selectedIndex].value
  console.log(year1)
  //return year1

  fetch(`/economyBowler?year=${year1}`)
    .then(r => r.json())
    .then(data => visualizeEconomyBowler(data, year1));
}
...
function visualizeData(data) {
  ...
  //visualizeEconomyBowler(data.economyBowler, year1);
...
function visualizeEconomyBowler(economyBowler, year1) {
  const seriesData = [];
  ...
  Highcharts.chart("economy-Bowler", {
    ...
    title: {
      text: `Top 10 Economical Bowlers in ${year1}`
    },
    subtitle: {
      text:
